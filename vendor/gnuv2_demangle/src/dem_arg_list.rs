    ...
    pub(crate) fn push<'s>(
        ...
        let arg = match arg {
            ...
            DemangledArg::Repeat { count, index } => {
                ...
                // A repeat expands to `count` entries in this vec; the count
                // is a plain number from the symbol, so a crafted one
                // (`N1255555555555555…`) would push unboundedly. Real
                // signatures repeat an argument a handful of times (C99 only
                // requires 127 parameters); anything past 65535 cannot be a
                // real encoding.
                if count.get() > 65535 {
                    return Err(DemangleError::InvalidRepeatingArgument(s));
                }
...
pub(crate) fn demangle_argument_list_impl<'c, 's, 'ns>(
    ...
    while !args.is_empty() && !args.starts_with('_') {
        ...
        args = remaining;
        if remaining.len() >= old_args.len() {
            return Err(DemangleError::NoProgressOnArgument(remaining));
        }
